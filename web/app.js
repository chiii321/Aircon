...
function renderOverview() {
  ...
  return `<div class="page overview-page"><div class="overview-brand"><img class="brand-symbol" src="assets/inuvair-logo-light.svg" alt=""><span class="brand-name">INUVAIR</span><span class="brand-caption">ROOM CLIMATE</span></div>
...
function renderAlerts() {
  ...
  const commandRows = commandHistory.map(c => `<article class="alert-detail"><span class="alert-mark" aria-hidden="true">${c.status === 'failed' ? '!' : 'i'}</span><div><strong>${esc(deviceName(c.device_id))} · ${esc(c.action.toUpperCase())} · ${commandStatus(c)}</strong><small>${esc(c.error_message || (c.status === 'sent_ir' ? 'IR transmission acknowledged; physical AC response is unverified.' : c.status === 'queued' ? 'Waiting for controller acknowledgement.' : 'Controller command update.'))}</small><small>${esc(formatDate(c.acknowledged_at || c.requested_at))}</small></div>${isAdmin() ? `<a class="subtle-link" href="#device/${encodeURIComponent(c.device_id)}">View device →</a>` : ''}</article>`).join('')
  return `<div class="page">${pageHead('System notices', 'Notifications', isAdmin() ? 'Connection and command issues from across the controller fleet.' : 'Connection, schedule check-ins, and command updates for devices assigned to you.')}
    ...
    <section class="card alert-list"><div class="card-heading"><div><div class="eyebrow">RECENT UPDATES</div><h2>Notification feed</h2></div><span class="alert-count">${attention.length + commandHistory.length}</span></div>${deviceRows}${commandRows}${!attention.length && !commandHistory.length ? `<p class="alert-clear">${devices.length ? 'No notifications right now.' : isAdmin() ? 'No controller notifications yet.' : 'No rooms are assigned to you at this time.'}</p>` : ''}<p class="alert-foot">Connection warnings use the latest heartbeat. Unconfigured controller slots are listed in Devices.</p></section></div>`
}
