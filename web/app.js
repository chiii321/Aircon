...
function renderOverview() {
  ...
  return `<div class="page">${pageHead('Control center', 'Your spaces at a glance.', 'Monitor every ESP32, then open a device for its controls and daily schedule.')}${banner('Live connection status', 'Online means a valid heartbeat arrived within 30 seconds. An IR acknowledgement confirms transmission was attempted; the physical AC state still needs separate verification.')}
    <div class="stats"><div class="card stat"><div class="stat-label">Online now</div><div class="stat-value">${online}</div><div class="stat-sub">Recent device heartbeats</div></div><div class="card stat"><div class="stat-label">Provisioned</div><div class="stat-value">${provisioned}<span class="muted"> / 11</span></div><div class="stat-sub">Controllers with credentials</div></div><div class="card stat"><div class="stat-label">Offline or pending</div><div class="stat-value">${11 - online}</div><div class="stat-sub">Review in Devices</div></div></div>
    <div class="section-heading"><h2>Controllers</h2><small>Click a row for full controls</small></div>${table(devices)}</div>`
}
