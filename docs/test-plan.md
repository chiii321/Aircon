...
For the reported no-response issue, first follow [IR troubleshooting](ir-troubleshooting.md). Firmware preserves the ON/OFF labels confirmed from actual AUX behavior. Replay with the temporary harvested-LED transistor circuit remains unverified; see [wiring.md](wiring.md).
...
4. Verify harvested LED polarity and the transistor wiring, check IR emission with a phone camera if possible, and start 10–20 cm from the AC receiver. Run `on` and `off` separately with LF, CR, and CRLF line endings. Each line should log one send. Check actual AC response and range, following the AC manual's restart interval. A Serial send message is not proof of AC operation.
