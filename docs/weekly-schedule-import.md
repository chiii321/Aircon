...
Imports add bookings; exact duplicate slots are skipped. They do not replace saved bookings. Remove an old booking from the saved list before importing its replacement. Deleting all bookings for a user leaves them without device access.
...
For every authorized account, database access rules allow only controllers in the current weekday/time slot: start is inclusive, end is exclusive. Accounts without a current booking have no device access. Manual assignments do not grant access. Admins retain full access. Approval is required for either assignment mode. Dashboard refresh normally updates visibility every eight seconds; an expiry timer clears displayed room information at the end of an active slot using server time. Admins can view each user’s booked rooms in a compact dropdown in User access. Class check-in responses require current room access.
