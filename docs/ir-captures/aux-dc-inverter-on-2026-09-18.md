...
User confirmation (2026-09-19): the ON label comes from actual AUX behavior observed during capture. Preserve these exact bytes and this label; do not reverse based on an inferred protocol bit. Replay with the temporary harvested-LED transistor circuit remains pending.

```cpp
uint8_t AUX_ON_STATE[13] = {
