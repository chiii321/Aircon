-- Admin status comes only from the private owner allowlist. Authorized users
-- are approved by an admin through a protected function and see only devices
...
as $$
  select exists (
    select 1
    from auth.users u
    join private.allowed_owners a on lower(a.email) = lower(u.email)
...
as $$
  select exists (
    ...
    where u.id = (select auth.uid())
      and u.raw_app_meta_data ->> 'inuvair_role' = 'authorized'
  );
$$;
...
revoke all on function private.can_access_device(text) from public, anon;
grant usage on schema private to authenticated;
grant execute on function private.is_admin() to authenticated;
grant execute on function private.is_authorized_user() to authenticated;
grant execute on function private.can_access_device(text) to authenticated;

drop policy if exists "owner reads devices" on public.devices;
create policy "admins and assigned users read devices" on public.devices
  for select to authenticated
  using ((select private.can_access_device(id)));

drop policy if exists "owner reads commands" on public.device_commands;
create policy "admins and assigned users read commands" on public.device_commands
  for select to authenticated
  using ((select private.can_access_device(device_id)));

drop policy if exists "owner queues commands" on public.device_commands;
create policy "admins queue commands" on public.device_commands
  ...
  with check (
    ...
    and exists (
      select 1 from public.devices d
      where d.id = device_id and d.provisioned
    )
  );

drop policy if exists "owner reads schedules" on public.device_schedules;
drop policy if exists "owner creates schedules" on public.device_schedules;
drop policy if exists "owner updates schedules" on public.device_schedules;
drop policy if exists "owner deletes schedules" on public.device_schedules;
create policy "admins manage schedules" on public.device_schedules
